...
def conf():
    ...
    argparser.add_argument("--input_folder", type=str, default="../../test_screendata/data")
    argparser.add_argument("--output_folder", type=str, default="../../test_screendata/output")
...
if __name__ == "__main__":
    ...
    output_folder_name = conf_file.stem
    output_folder_path = Path(f"{args.output_folder}/{output_folder_name}")
    ...
    summary_f = f"{args.output_folder}/{output_folder_name}/summary.json"
    output_dir = str(output_folder_path)
    input_folder = Path(args.input_folder)
    start = time()
    with open(summary_f, "w") as f:
        img_full_paths  = [str(img) for img in input_folder.glob("*.png")]
